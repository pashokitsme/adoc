...
страницы печатается везде, где сайт его даёт. В таблицах (`part`, `analogs`,
`search`, «уточни бренд», корзина) адреса идут **списком сразу под таблицей**,
и номер слева — тот же номер, что в колонке `#`: URL длиной с полстроки
терминала в колонке разорвал бы вёрстку каждой строки, а обрезанный адрес
бесполезен. Одинаковый адрес второй раз не печатается: у десятка предложений
одной детали карточка одна.

Страница целиком — корзина, страница отзывов, карточка в `info`, список
заказов — стоит отдельной строкой у своего заголовка.

Строка `search` или «уточни бренд» может лежать сразу на нескольких сайтах, а
в списке у неё один адрес: остальные идут ниже блоком «ещё ссылки», где рядом
с номером написано, чей это сайт.

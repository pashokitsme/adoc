...
Цену и срок из таблицы всё равно идут проверять на сайте, поэтому адреса
страниц печатаются везде, где сайт их отдаёт: карточки предложений в `part` и
`analogs`, товары в `search`, варианты в «уточни бренд», позиции корзины,
страница отзывов и сами отзывы, карточка в `info`.

Адрес в колонку не ставится: он длиной с полстроки терминала и разорвал бы
вёрстку на каждой строке, а обрезанный URL бесполезен — по нему не открыть
страницу. Поэтому ссылки идут списком **под** таблицей, и номер слева в нём —
тот же номер, что в колонке `#`. Одна строка выдачи может иметь несколько
адресов: товар, который нашёлся у двух сайтов, лежит на двух разных страницах,
и в списке они обе, каждая со своим сайтом.

Адрес самой страницы — корзины, страницы отзывов, карточки — стоит в заголовке
блока, рядом с именем сайта. В `--json` ссылки лежат прямо в объектах, полем
`url` (и `urls` — у склеенных строк `search`).
...
сайта. У кого такой привязки нет, тот ищет без машины и говорит об этом
жёлтой строкой; какая машина участвовала и у кого — написано над таблицей.

...
armtek     1  VAG    Болт      380 ₽  3 шт     2 дня   armtek    4.5★ (10)  https://armtek.ru/product/…
autodoc    2  VAG    Болт      407 ₽  12 шт    1 день  Москва    4.9★ (56)  https://www.autodoc.ru/…
```
...
страницы печатается везде, где сайт его даёт: колонкой `ССЫЛКА` в `part`,
`analogs`, `search`, «уточни бренд» и корзине; строкой рядом с заказом в
`orders`; отдельной строкой под карточкой в `info` и под оценками в `reviews`.
Один и тот же адрес второй раз в таблице не печатается: у десятка предложений
одной детали карточка одна, и колонка остаётся узкой.

В колонку помещается один адрес, а строка `search` или «уточни бренд» может
лежать сразу на двух сайтах: вторая ссылка уезжает в блок «ещё ссылки» под
таблицей, и номер слева в нём — тот же, что в колонке `#`.

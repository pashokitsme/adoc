...
import { TOOL, dim, need, renderProducts } from "../sdk/index.ts"
import { limitOf, pageOf } from "../core/args.ts"
import { chooseCar, refsOf, tecdoc } from "../core/car.ts"
import { carLabel } from "../core/garage.ts"
...
export async function cmdSearch(ctx: Ctx): Promise<Output> {
	...
	return {
		...
		render: () => [
			...
			// иначе он обещал бы подбор, которого не было. И только имя машины и
			// сайты: у иных модификаций одно название в полстроки, поэтому совет
			// про --no-car живёт внизу, вместе с остальными подсказками.
			...(car && used.length

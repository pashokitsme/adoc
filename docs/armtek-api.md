...
Ограничение висит на том, кто спрашивает: проверено вживую — тот же запрос с
гостевым токеном отвечает `200`, а с токеном вошедшего аккаунта `429`, и
заголовки браузера (`User-Agent`, `Origin`, `Sec-Fetch-*`) этого не меняют.
Поэтому публичное чтение (поиск, бренды, предложения, карточка, аналоги,
отзывы, точки выдачи) при 429 повторяется **один раз** гостевым токеном, а в
stderr уходит строка «armtek: аккаунт ограничен сайтом, цены показаны как для
гостя»: цена, срок и наличие зависят от договора, и подменять их молча нельзя.
Корзина, заказы и гараж гостю не видны вовсе — они остаются на токене аккаунта
и отвечают тем же 429.

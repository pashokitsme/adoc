...
У armtek есть предел частоты: на слишком частые запросы он отвечает «слишком
много запросов подряд — сайт просит подождать». Это обычный отказ одного сайта
(код `http`, выдача остальных на месте, exit 0) — подожди пару минут и повтори,
а не бомби его в цикле.

...
Ссылки на страницы сайтов есть в каждой выдаче: колонкой `ССЫЛКА` в таблицах
(`part`, `analogs`, `search`, «уточни бренд», корзина), строкой рядом с
заказом в `orders`, отдельной строкой под карточкой в `info` и под оценками в
`reviews`. Повторный адрес в таблице не печатается — у десятка предложений
одной детали карточка одна. Строка, лежащая сразу на двух сайтах, отдаёт
вторую ссылку в блок «ещё ссылки» под таблицей (номер слева = колонка `#`).
В `--json` это поле `url` объекта, а у склеенных строк `search` и брендов —
ещё и `urls` (сайт → адрес). Показывай их пользователю: он идёт по ним на сайт.

...
`createDate` и один товар в `goods`. Умолчание — последний месяц. Позиции
одного заказа отличить по ответу нельзя (общего номера в нём нет), поэтому
`orders` отдаёт по одному `Order` на позицию — так же, как их показывает и
отменяет сам сайт.
...
| `orders` | сырой JSON | контрактная `orders`: номер, дата, статус, сумма, позиции и ссылки |
...
- У autodoc `orders` отдаёт по одному `Order` на позицию заказа. Если сайт
  где-то отдаёт настоящий номер заказа (`orders/grouped-history/{id}`,
  `orders/details/{id}`), позиции можно будет сгруппировать.
